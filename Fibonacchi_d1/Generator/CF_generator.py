...
def cf_transform(atoms):
    ...
    list is our fibonacci string. In program I use sections lengths: L = tau = 1.618... and S = 1. More about that
    ...
    df = pd.DataFrame({'Counts L': count_l,
                       'Counts S': count_s,
                       'Fibonacci string': num_fibo})
    df.to_csv('fibo_transform.csv')
...
def cf_projection(length):
    ...
    are correct More about that idea you can read in my engineer's thesis in  chapter 'Metoda generacji ciągu
